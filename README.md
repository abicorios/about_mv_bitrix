...
User bitrix
```
Если доступ известен только по ip, в конфиге вместо `abc.def.ghi` нужно будет напечатать `192.168.167.166`.
Это очень удобная настройка, после которой можно логиниться на сервер командой `ssh server` и передавать файлы с сервера на локальный компьютер через удобный синтаксис `rsync -az server:log.txt log.txt`. Если же конфиг не настроен, то нужно будет в командной строке указывать целый ряд параметров, и в случае `rsync` придётся использовать крайне неудобный синтаксис. Поэтому лучше настроить кофиг.
