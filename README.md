...
Иногда полезно знать что ключ пуш-сервера хранится в файле `/etc/sysconfig/push-server-multi`, это главный файл. Такой же ключ должен быть в `bitrix/.settings.php` и в настройках модуля в административном разделе. При копировании этого ключа легко ошибиться. Бывает нужно просто сохранить настройки модуля. Также выглядит естественным в настройках модуля отключить отправку уведомлений на мобильные телефоны.
